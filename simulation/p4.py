...
def datos_elipse(xpos, ypos):
    ...
    sema = round(dist(xmin, xmax) / 2, 5)
    seme = round(dist(ymin, ymax) / 2, 5)

    exc = round(np.sqrt(sema**2 - seme**2) / sema, 2)

    p = round(np.sqrt(sema ** 3), 5)
...
for i in range(30):
    ...
    for i in range(200000):
        ...
        vpos.append(v)
        xpos.append(round(xc, 5))
        ypos.append(round(yc, 5))
        tpos.append(tc)
        euler.do_step()
    ...
    infoelipses.append((round(aux1, 5), round(aux2, 5), round(sema, 5), round(p, 5)))
    cont2 += round(0.05, 2)
# print("Sema =", sema, ", Seme =", seme)
# print("Excent =", exc, ", Periodo =", p)
...
# semapos = [0.5328, 0.5594, 0.6134, 0.6686, 0.7252, 0.7834, 0.8437,
#            0.9063, 0.9717, 1.0403]
# ppos = [0.3889, 0.4184, 0.4804, 0.5467, 0.6175, 0.6935, 0.7750, 0.8629,
#         0.9576, 1.0611]

# fig, ax = plt.subplots()
# ax.plot(ppos, semapos, '--', label=m2)
#
# ax.set(xlabel='p (yr)', ylabel='sema (a.u.)',
#        title='Simulation Planet Motion, deltat: ' + str(deltat))
#
# ax.grid()
# plt.legend()
