...
W = 0.
W2 = 0.6 # 3% de la ace, asi a = v**2 / r, a = (vx**2 + vy**2) / np.sqrt(x**2 + y**2)
         # a = 20, 3% de 20 es 0.6
print("Con W =", W)
...
def universal_gravity(state, params):
    ...
    r2 = x**2 + y**2
    r3 = np.sqrt(r2) * r2
    ax = (-gu * x / r3) + W * np.cos(np.radians(45))
    ay = (-gu * y / r3) + W * np.sin(np.radians(45))
...
plt.legend()
plt.savefig("CemtConW.PNG")
plt.show()


# Observe que con x0 = 1.5, y0 = 1, vx = 0, vy = 1.5, deltat = 0.01/128, W = 0
# tenemos una orbita eliptica, grafica orbitaElipSinW

# Ahora con W = 0.6, el satelite hace 13 orbitas, luego se estrella con la
# tierra, grafica orbitaElipSinW, ademas de la misma grafica con zoom
